...
from openmm import Platform, LangevinIntegrator, MonteCarloBarostat, unit
from openmm.unit import nanometer, picosecond, kelvin, femtosecond
from openmm.app import PDBFile, StateDataReporter
from mdtraj.reporters import XTCReporter, DCDReporter, HDF5Reporter
import mdtraj as md
...
class ProteinDownloader:
    ...
    def process_pdb_with_residue_validation(self, fix_partial_prot):
        for prot in [self.prot1, self.prot2]:
            ...
            fixer = PDBFixer(filename=f"{str(prot.pdb_path)[:-4]}_fixed.pdb")
    ...
    def _fix_clashes(self, fixed_pdb_path):
        ...
        system = forcefield.createSystem(pdb.topology, nonbondedCutoff=1.0* nanometer)
    ...
    def _add_fixer_tag(self, prot):
        ...
        with open(f"test_out.pdb", 'w') as f:
            # Write SEQRES section
            f.writelines(seqres_lines)
            # Write original PDB lines
            f.writelines(pdb_lines)
...
class OpenMMSimulation:
    ...
    def martinize_files(self, pair, files_path, sim_type: str):
        os.makedirs("CG_OpenMM", exist_ok=True)
        #os.chdir("CG_OpenMM")
        
        if sim_type == "Single":
            if os.path.exists(f"CG_OpenMM/{self.prot1.name}_martinized.gro"):
                logger.info(f"{self.prot1.name} already Martinized. Skipping...")
                return
            
            logger.info(f"Martinizing {self.prot1.name}...")
            command = f"martinize2 -f pdb_fa_files/{self.prot1.name}_{self.prot1.species}_fixed.pdb -o CG_OpenMM/{self.prot1.name}.top -x CG_OpenMM/{self.prot1.name}_martinized.pdb -resid input -ignh -maxwarn 2"
            ...
            if process.returncode != 0:
                ...
                logger.info("Martinizing completed successfully.")
            
            pdb = md.load(f'CG_OpenMM/{self.prot1.name}_martinized.pdb')
            pdb.save(f'CG_OpenMM/{self.prot1.name}_martinized.gro')
            self._deal_with_gro_files(f'CG_OpenMM/{self.prot1.name}_martinized.gro')

        elif sim_type == "Dimer":
            if os.path.exists(f"CG_OpenMM/{self.prot1.name}_{self.prot2.name}_martinized.gro"):
                logger.info(f"{self.prot1.name}_{self.prot2.name} already Martinized. Skipping...")
                return
            
            logger.info(f"Martinizing {self.prot1.name} and {self.prot2.name}...")
            command = f"martinize2 -f CG_OpenMM/{self.prot1.name}_{self.prot2.name}_docked.pdb -o CG_OpenMM/{self.prot1.name}_{self.prot2.name}.top -x CG_OpenMM/{self.prot1.name}_{self.prot2.name}_martinized.pdb -resid input -ignh -maxwarn 4"
            ...
            pdb = md.load(f'CG_OpenMM/{self.prot1.name}_{self.prot2.name}_martinized.pdb')
            pdb.save(f'CG_OpenMM/{self.prot1.name}_{self.prot2.name}_martinized.gro')
            self._deal_with_gro_files(f'CG_OpenMM/{self.prot1.name}_{self.prot2.name}_martinized.gro')

        # Move Martinized molecule file
        if os.path.exists("molecule_0.itp"):

            with open("molecule_0.itp", 'r', encoding='utf-8') as file:
                lines = file.readlines()

            with open("molecule_0.itp", 'w', encoding='utf-8') as file:
                file.writelines(lines[7:])
            
            shutil.move("molecule_0.itp", f"CG_OpenMM/molecule_0.itp")

        if os.path.exists("molecule_1.itp"):

            with open("molecule_1.itp", 'r', encoding='utf-8') as file:
                lines = file.readlines()

            with open("molecule_1.itp", 'w', encoding='utf-8') as file:
                file.writelines(lines[10:])
            
            shutil.move("molecule_1.itp", f"CG_OpenMM/molecule_1.itp")

        if os.path.exists("molecule_2.itp"):

            with open("molecule_2.itp", 'r', encoding='utf-8') as file:
                lines = file.readlines()

            with open("molecule_2.itp", 'w', encoding='utf-8') as file:
                file.writelines(lines[13:])
            
            shutil.move("molecule_2.itp", f"CG_OpenMM/molecule_2.itp")

               
    
    def _deal_with_gro_files(self, gro_file_path):
        atom_positions = []
        with open(gro_file_path, 'r') as file:
            lines = file.readlines()
            for line in lines[2:-1]:  # Skip the first two lines and the last line
                tokens = line.split()
                if len(tokens) >= 6:  # Ensure the line has enough tokens
                    atom_positions.append((float(tokens[3]), float(tokens[4]), float(tokens[5])))
        self._calculate_box_vectors(gro_file_path, atom_positions)

    def _calculate_box_vectors(self, gro_file_path, atom_positions, buffer=10.0):
        min_coordinates = [min(coord) for coord in zip(*atom_positions)]
        max_coordinates = [max(coord) for coord in zip(*atom_positions)]

        # Add buffer to the box dimensions
        min_coordinates = [coord - buffer for coord in min_coordinates]
        max_coordinates = [coord + buffer for coord in max_coordinates]

        # Calculate box vectors
        box_vectors = [max_coord - min_coord for min_coord, max_coord in zip(min_coordinates, max_coordinates)]
        
        self._update_gro_box(gro_file_path, box_vectors)

    def _update_gro_box(self, gro_file_path, box_vectors):
        with open(gro_file_path, 'r') as file:
            lines = file.readlines()
        
        # Update the last line with box vectors
        lines[-1] = f"{' '.join([str(dim) for dim in box_vectors])}\n"

        with open(gro_file_path, 'w') as file:
            file.writelines(lines)
        
        logger.info(f"Updated .gro file saved as {gro_file_path}")

    def run_simulation(self, sim_type: str):
        os.chdir("..")
        shutil.copyfile("martini.itp", f"{self.prot1.name}_{self.prot1.species}_{self.prot2.name}_{self.prot2.species}/CG_OpenMM/martini.itp")
        os.chdir(f"{self.prot1.name}_{self.prot1.species}_{self.prot2.name}_{self.prot2.species}/CG_OpenMM")
        ...
        conf = GromacsGroFile(f"{file_name}_martinized.gro")
        ...
        friction_coeff = 5 / unit.picosecond  # This controls the damping
        ...
        equi_state_file = f'{work_dir}/equi.state'
        equi_checkpoint_file = f'{work_dir}/equi.chk'

        # Run equilibration
        if not (os.path.exists(equi_state_file) and os.path.exists(equi_checkpoint_file)):
            # NVT equilibration
            ...
            # NPT equilibration
            simulation.system.addForce(MonteCarloBarostat(1 * bar, temperature))  # Match temperature
            logger.info('Running NPT equilibration...')
            simulation.step(50000)

        # Set up reporters
        simulation.reporters.append(StateDataReporter(f"{work_dir}/prod.log", 1000,
                                                    step=True,
        ...
        simulation.reporters.append(DCDReporter(f'{work_dir}/trajectory.dcd', 1000))
        
        # Set up XTC reporter
        xtc_reporter = XTCReporter(f'{work_dir}/prod.xtc', 1000)
        simulation.reporters.append(xtc_reporter)

        simulation.saveState(equi_state_file)
        simulation.saveCheckpoint(equi_checkpoint_file)
        ...
        for prot1_residue, prot2_residue in residue_pairs:
            ...
            if aa1 == aa2 and sim_type == "Single":
                ...
                sim_rejected_residues.append([idx, prot1, peptide1, aa1, prot2, peptide2, aa2, copies, crosslinker, "0", max_dist, "Same residue: Likely a homodimer case", original_file_path])
                continue

            logger.info(f"Working on residues: {aa1} and {aa2}")

            atom1, atom2 = self._extract_bb_atoms_from_pdb(f"{file_name}_martinized.pdb", aa1, aa2, sim_type)
            distance = self._compute_distance(simulation, atom1, atom2, sim_type)
            initial_distance = distance
            logger.info(f"Initial distance: {initial_distance} nm")
            logger.info(f"Max distance set to: {max_dist/10} nm")
            ...
            if distance > (max_dist / 10):
                ...
                # Run simulation until distance is under max, or step limit is reached
                steps_taken = 0
                while distance > (max_dist / 10) and steps_taken < step_limit:
                    ...
                    logger.info(f"Step: {steps_taken}, Distance: {distance} nm")
                    #logger.info(f"Potential energy: {state.getKineticEnergy()}     , Kinetic energy: {state.getPotentialEnergy()}")
                    additional_steps = 0
                    while distance > initial_distance and additional_steps < 10000:
                        simulation.step(1000)
                        additional_steps += 1000
                        distance = self._compute_distance(simulation, atom1, atom2, sim_type)
                    
                    if additional_steps != 0:
                        logger.info(f"Additional steps taken to normalize context. Additional Steps: {additional_steps}")
                        additional_steps = 0
                ...
                if steps_taken <= step_limit and distance <= (max_dist / 10):
                    sim_accepted_residues.append([idx, prot1, peptide1, aa1, prot2, peptide2, aa2, copies, crosslinker, distance*10, max_dist, f"Accepted after {steps_taken} steps", original_file_path])
                    ...
                    sim_rejected_residues.append([idx, prot1, peptide1, aa1, prot2, peptide2, aa2, copies, crosslinker, distance*10, max_dist, "Reached max steps of 5000", original_file_path])
                    logger.info(f"Step limit reached for residues {aa1} and {aa2}.")
                    continue
            
            else:
                logger.info(f"Initial distance is already under max distance.")
                sim_accepted_residues.append([idx, prot1, peptide1, aa1, prot2, peptide2, aa2, copies, crosslinker, distance*10, max_dist, "Pre-sim accepted", original_file_path])

        # Save final structure and results
        self._unwrap_trajectory(f"{file_name}_martinized.pdb", f"{work_dir}/trajectory.dcd", f"{work_dir}/trajectory_unwrapped.dcd")
    ...
    def _unwrap_trajectory(self, pdb_file, input_dcd, output_dcd):
        if not os.path.exists(input_dcd):
            return
    ...
    def _extract_bb_atoms_from_pdb(self, martinized_pdb, aminoacid_number1, aminoacid_number2, sim_type):
        bb_atoms1 = []
        bb_atoms2 = []
        pdb_atom_count = -1     # To account for 0-based indexing

        with open(martinized_pdb, 'r') as f:
            for line in f:
                if line.startswith("ATOM"):             # Only consider ATOM records, as TER records may be present
                    pdb_atom_count += 1                 # Manual count of ATOM records
                    atom_name = line[12:16].strip()
                    chain_id = line[21]
                    ...
                    if sim_type == "Single":
                        if chain_id == 'A':
                            if residue_number == aminoacid_number1:
                                bb_atoms1.append(pdb_atom_count)
                            elif residue_number == aminoacid_number2:
                                bb_atoms2.append(pdb_atom_count)
                    elif sim_type == "Dimer":
                        if chain_id == 'A' and residue_number == aminoacid_number1:
                            bb_atoms1.append(pdb_atom_count)
                        elif chain_id == 'B' and residue_number == aminoacid_number2:
                            bb_atoms2.append(pdb_atom_count)

        if not bb_atoms1 or not bb_atoms2:
            raise ValueError(f"BB atoms not found for residues {aminoacid_number1} and/or {aminoacid_number2}")

        return bb_atoms1[-1], bb_atoms2[-1]

    def _compute_distance(self, simulation, atom_index1, atom_index2, sim_type: str):
        positions = simulation.context.getState(getPositions=True).getPositions()

        pos1 = positions[int(atom_index1)]
        pos2 = positions[int(atom_index2)]
    ...
    def _save_final_structure(self, simulation, work_dir):
        output_pdb_filename = f'{work_dir}/{self.prot1.name}_{self.prot2.name}_post_sim_martinized.pdb'            
    ...
    def _save_results(self, sim_accepted_residues, sim_rejected_residues, work_dir, sim_type):
        ...
        with open(f"{work_dir}/CG_Simulation_accepted_residues_{self.prot1.name}{prot2_name}.csv", 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["IDx", "Protein1", "Peptide1", "AA1", "Protein2", "Peptide2", "AA2", "Copies",
                              "Crosslinker", "Final Distance", "Max Dist", "Notes", "Original File Path"])
...
class CrossLinkAnalyzer:
    ...
    def _write_sasa_files(self, input_file, output_file):
        warnings.filterwarnings("ignore", category=UserWarning, module="freesasa")
...
def main():
    ...
    for pair, data_list in protein_pairs.items():
        ...
        if protein1 == protein2 and not args.skip_simulation1:
            ...
            OpenMM_Sim.martinize_files(pair, files_path, sim_type="Single")
        ...
        if not args.skip_simulation2:
            ...
            OpenMM_Sim.martinize_files(pair, files_path, sim_type="Dimer")
            OpenMM_Sim.run_simulation(sim_type="Dimer")

        os.chdir("..")

    #Fetching of final results goes here
    FinalResultVisualizer()
